...
def doAnisoAndBiascorrection(binaryName,i):
            ...
            biasName =  "./Output/Normalisation/BiasField/biasfield"+str(i)+".nii"
...
def normalize(location, Modality):
        ...
        for dirpath, dirnames, filenames in os.walk(location):
            for filename in [f for f in filenames if ((Modality in f)and('.nii'in f)) ]:
                ...
                i= i+1      
        print("do Nyul model")
        outputModel = './Output/Normalisation/nyulModel/nyulModel'+Modality
        ...
        for dirpath, dirnames, filenames in os.walk(location):
            for filename in [f for f in filenames if (('.nii' in f)and(Modality in f))]:
                ...
                directory = './Output/Normalisation/Normalisation/'+p.parts[2]+'/'
...
if __name__ == "__main__":
    ...
    directory = './Output/Normalisation'
    if not os.path.exists(directory):
        os.makedirs(directory)

    directory = './Output/Normalisation/BiasField'
    if not os.path.exists(directory):
        os.makedirs(directory)

    directory = './Output/Normalisation/nyulModel'
    if not os.path.exists(directory):
        os.makedirs(directory)

    directory = './Output/Normalisation/Normalisation'
    if not os.path.exists(directory):
        os.makedirs(directory)
